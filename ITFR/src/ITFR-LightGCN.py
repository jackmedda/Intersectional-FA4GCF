...
import fa4gcf.data.utils as fa4gcf_data_utils
import fa4gcf.utils as fa4gcf_utils
...
class MF(nn.Module):
    def __init__(self, data_config):
        ...
        self.n_layers = data_config['gcn_layers']
        ...
        self.edge_index, self.edge_weight = get_norm_adj_mat(data_generator.all_inter, self.n_users, self.n_items, enable_sparse=True)
        self.edge_index, self.edge_weight = self.edge_index.to("cuda"), self.edge_weight.to("cuda") if self.edge_weight is not None else None
    ...
    def get_ego_embeddings(self):
        ...
        """
        user_embeddings = self.weights["user_embedding"]
        item_embeddings = self.weights["item_embedding"]
    ...
    def forward(self):
        ...
        for layer_idx in range(self.n_layers):
            all_embeddings = self.weights['gcn_conv'](all_embeddings, self.edge_index, self.edge_weight)
    ...
    def _pre(self, users, pos_items):
        ...
        with torch.no_grad():
            ...
            u_e = torch.index_select(user_embeddings, 0, users)
            pos_i_e = torch.index_select(item_embeddings, 0, pos_items)
    ...
    def _init_weights(self):
        ...
        all_weights['gcn_conv'] = LightGCNConv(dim=self.emb_dim)
    ...
    def epoch_end_process(self):
        for g in range(self.group_num):
            self.loss_after[g] = np.average(self.cum_loss_list[g], weights=self.cum_num_list[g]).astype(float)
...
def get_norm_adj_mat(inter_feat, user_num, item_num, enable_sparse=False):
    ...
    row = torch.LongTensor(inter_feat[:, 0])
    col = torch.LongTensor(inter_feat[:, 1] + user_num)
    ...
    return fa4gcf_data_utils.get_norm_adj_mat(
        edge_index,
        edge_weight,
        num_nodes,
    ...
    for epoch in range(args.epoch):
        ...
        for batch in dataloader:
            ...
            for li in range(model.group_num):
                ...
                group_users = torch.index_select(user_embeddings, 0, users[gidx])
                group_pos_items = torch.index_select(item_embeddings, 0, pos_items[gidx])
                group_neg_items = torch.index_select(item_embeddings, 0, neg_items[gidx])
